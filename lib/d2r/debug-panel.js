...
class DebugPanel {
  constructor(objectManager) {
    ...
    for (let i = 0; i < TYPE_COUNT; i++) this._nodes[i] = new Map();
    ...
    for (let i = 0; i < TYPE_COUNT; i++) {
      this._headers[i] = new gui.CollapsingHeader(`${TYPE_NAMES[i]} (0)`);
  ...
  _updateHeaderLabel(type) {
    this._headers[type].label = `${TYPE_NAMES[type]} (${this._nodes[type].size})`;
  }
  ...
  refresh() {
    ...
    this._summary.text = parts.length > 0 ? parts.join(' | ') : 'No units';
  ...
  _unitLabel(unit) {
    return `#${unit.id} classId=${unit.classId} mode=${unit.mode} @ (${unit.posX}, ${unit.posY})`;
  }
